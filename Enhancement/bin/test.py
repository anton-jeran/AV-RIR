...
class TestGEN(abc.ABC):
    ...
    @abc.abstractmethod
    def encode(self, x,material,color):
        pass
    ...
    def run(self):
        ...
        with torch.no_grad(), tqdm(self.dataset, desc="[test]") as pbar:
            ...
            for idx, (utt_id, x_list) in enumerate(pbar, 1):
                x,material,color = x_list
                print("x shape ",x.shape)
                start = time.time()
                zq_speech, zq_rir = self.encode(x,material,color)
                ...
                # input("summa")
                clean1 = os.path.join(clean_dir,utt_id[0])
                rir1 = os.path.join(rir_dir,utt_id[0])
                reverb1 = os.path.join(reverb_dir,utt_id[0])

                if(not os.path.exists(clean1)):
                    os.mkdir(clean1)

                if(not os.path.exists(rir1)):
                    os.mkdir(rir1)

                if(not os.path.exists(reverb1)):
                    os.mkdir(reverb1)

                self._save_wav(os.path.join(clean1, f"{utt_id[1]}.wav"), y_speech)
                self._save_wav(os.path.join(rir1, f"{utt_id[1]}.wav"), y_rir)
                self._save_wav(os.path.join(reverb1, f"{utt_id[1]}.wav"), y_reverb_speech_)
