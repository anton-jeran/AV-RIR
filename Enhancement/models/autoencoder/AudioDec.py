...
class Generator(torch.nn.Module):
    ...
    def forward(self, x,mi,ci):
        ...
        (batch_m, color_m, channel_m, length_m) = mi.size()
        (batch_c, color_m, channel_c, length_c) = ci.size()


        if channel != self.input_channels: 
            x = x.reshape(-1, self.input_channels, length) # (B, C, T) -> (B', C', T)
        x_speech, x_rir = self.encoder(x,mi,ci)
        ...
        # print("y_speech shape  ",y_speech.shape)
