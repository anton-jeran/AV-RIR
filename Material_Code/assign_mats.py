...
abs_oct = dict()
mat_folder = "/fs/nexus-scratch/jeran/GWA/tools/"
# load material list and create database if not already exist
mat_json = "../files/acoustic_absorptions.json"
...
pickle_file = "/fs/nexus-scratch/jeran/image/material_name.pickle"
...
material_name = list(data_dict.keys())
print("material name  ",material_name)
cur_embeddings = model.encode(material_name)
cosine_scores = cosine_similarity(cur_embeddings, full_embeddings)

mat_files_dict = {} 
mtl_dict = {}
for i, word in enumerate(material_name):
    if word in ['other', 'bounds']:
        chosen_ind = random.randrange(len(full_names))
    else:
        pos_ind = np.argwhere(cosine_scores[i] == max(cosine_scores[i]))[:, 0]
        if len(pos_ind):
    ...
    print("word ",word, "  name  ",save_names[full_names[chosen_ind]],"  abs  ",abs_oct[full_names[chosen_ind]])
    mtl_dict[word] = abs_oct[full_names[chosen_ind]]

with open("material_name.pickle", 'wb') as outfile1:
    pickle.dump(mat_files_dict, outfile1)

with open("material_absorption.pickle", 'wb') as outfile2:
    pickle.dump(mtl_dict, outfile2)
