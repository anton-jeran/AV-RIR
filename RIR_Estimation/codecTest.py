...
class TestMain(TestGEN):
    ...
    def load_dataset(self, subset, subset_num):
        ...
        audio_rs_dir = os.path.join(
             self.encoder_config['data']['subset'][subset],self.encoder_config['data']['reverb_path'])
        audio_material_dir = os.path.join(
             self.encoder_config['data']['subset'][subset],self.encoder_config['data']['material_path'])
        audio_image_dir = os.path.join(
             self.encoder_config['data']['subset'][subset],self.encoder_config['data']['image_path'])

        print("audio_rs_dir  ",audio_rs_dir )

        assert os.path.exists(audio_rs_dir), f"{audio_rs_dir} does not exist!"
        assert os.path.exists(audio_material_dir), f"{audio_material_dir} does not exist!"
        assert os.path.exists(audio_image_dir), f"{audio_image_dir} does not exist!"
    ...
    def encode(self, audio,material,color):
        ...
        # print("test mi shape ",mi.shape)
        # print("test ci shape ",ci.shape)

        if self.multi_channel:
            x = x.transpose(1, 0).unsqueeze(0) # (T, C) -> (1, C, T)
        else:
            x = x.transpose(1, 0).unsqueeze(1) # (T, C) -> (C, 1, T)
        x_speech, x_rir = self.encoder.encoder(x,mi,ci)
    ...
    def initial_folder(self, subset, output_name):
        ...
        # print("setdir  ",setdir)
        self.outdir = os.path.join(output_name, subset)
